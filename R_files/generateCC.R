...
more_colors <- colorspace::rainbow_hcl(20)
secondColor.map <- function(CATEGORY) {more_colors[CATEGORY]}
allColors <- unlist(lapply(new_d$fit.cluster, secondColor.map))
rowcolors <- allColors[rownames(new_cluster)]

filename = paste0("metsK20-k",numClusters,"_",format(Sys.time(), "%m-%e-%y_%H%M"),".jpg")
...
heatmap.2(as.matrix(new_cluster),
          ...
          trace = "none",
          col = rev(redblue(100)),
          main = paste0("Mets,kmean:k20, ConsensusClusters k=",numClusters),

...
cn_table$cn_PIK3CA <- ifelse(cn_table$cn_PIK3CA > 4, 1, 0)

cn_table <- merge(allSamples, cn_table, by = 0)
rownames(cn_table) <- cn_table$sample_ID
...
freqTable$percent <- NA
freqTable$fisher2 <- NA
for (row in 1:nrow(freqTable)){
  if (freqTable[row,"temp"] == "COLD"){
    freqTable[row, "percent"] <- (freqTable[row, "freq"])/numCold
    freqTable[row, "fisher2"] <- numCold - (freqTable[row, "freq"])
  }
  if (freqTable[row,"temp"] == "HOT"){
    freqTable[row, "percent"] <- (freqTable[row, "freq"])/numHot
    freqTable[row, "fisher2"] <- numHot - (freqTable[row, "freq"])
  }
}

fishTable <- freqTable[,c("temp","Name","freq","fisher2")]

resultsList <- list()
for (status in unique(fishTable$Name)){
  fTab <- fishTable[fishTable$Name == status,]
  fTab <- transform(fTab, row.names = temp, temp = NULL)
  if (nrow(fTab) == 1){
    resultsList <- c(resultsList, paste0(unique(fTab$Name),": Only ",rownames(fTab)))
    print(paste0(unique(fTab$Name),": Only ",rownames(fTab)))
  }
  else{
    ftest <- fisher.test(fTab[2:3])
    resultsList <- c(resultsList, paste0(unique(fTab$Name),": ",round(ftest$p.value,3)))
    print(paste0(unique(fTab$Name),": ",round(ftest$p.value,3)))
    }
}
...
p <-  p + geom_text(aes(temp, total + .05, label = t_label, fill = NULL), data = totals)

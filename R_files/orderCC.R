...
plotCC <- function(k_init, results, myclusters, cluster_map){
  ...
  for (num in seq(2,20,2)){
    ...
    for (cclus in 1:num){
      numRows <- length(c_cluster[4:ncol(c_cluster)][,c_cluster$consensusClass == cclus])
      TOTAL_SIG <- sum(c_cluster[4:ncol(c_cluster)][c_cluster$consensusClass == cclus,])/numRows
      c_cluster$heatScore[c_cluster$consensusClass == cclus] <- TOTAL_SIG
      }
    ...
    c_cluster <-arrange(c_cluster, heatScore)

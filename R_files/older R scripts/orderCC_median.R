...
plotCC <- function(k_init, results, myclusters, cluster_map){
  ...
  for (num in c(13)){
    ...
    coldSRRs <<- rownames(c_cluster[c_cluster$heatOrder == 1 |c_cluster$heatOrder == 2 |
                                        c_cluster$heatOrder == 3 | c_cluster$heatOrder == 4 |
                                        c_cluster$heatOrder == 5 | c_cluster$heatOrder == 6,])
    
    # write.table(rownames(c_cluster[c_cluster$heatOrder == 1 |c_cluster$heatOrder == 2 |
    #                                  c_cluster$heatOrder == 3 | c_cluster$heatOrder == 4 |
    #                                  c_cluster$heatOrder == 5 | c_cluster$heatOrder == 6,]),
    #             file = 'COLDtumors.csv',
    #             sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE)
    
    hotSRRs <<- rownames(c_cluster[c_cluster$heatOrder == (num-3)
                                      |c_cluster$heatOrder == (num-2)
                                      |c_cluster$heatOrder == num ,])

    # write.table(rownames(c_cluster[c_cluster$heatOrder == (num-1) |c_cluster$heatOrder == num ,]),
    #             file = 'HOTtumors.csv',
    #             sep = ",", row.names = FALSE, col.names = TRUE, quote = FALSE)
    # 
